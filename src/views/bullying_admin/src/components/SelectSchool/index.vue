<template>
  <div>
    <div class="search_criteria">
      <el-select v-model="search_school.province_code" class="select_provinces" placeholder="选择省份" clearable @change="handleGetNext(1)">
        <el-option
      ...
      <el-select v-model="search_school.city_code" class="select_provinces" placeholder="选择地市" clearable @change="handleGetNext(2)">
        <el-option
      ...
      <el-select v-model="search_school.county_code" class="select_provinces" clearable placeholder="选择县区">
        <el-option
      ...
      <el-input v-model="search_school.scl_name" style="width: 150px;margin-right: 8px" placeholder="学校名称" />
    ...
    <el-table ref="school" :data="school_list" :height="500" size="mini" strip>
      ...
      <el-table-column label="操作" align="center" width="100">
        <template slot-scope="scope">
          <span class="cut-btn" @click="singleElection(scope.row)">{{ type === 1 ? '切换' : '选择' }}</span>
          <!--<el-radio
  ...
  methods: {
    ...
    handleGetNext(type) {
      httpRequest('/api/sys/area/getNext', {
        areaPcode: type === 1 ? this.search_school.province_code : this.search_school.city_code
      }, false).then(res => {
        if (type === 1) {
          this.search_school.city_code = ''
          this.search_school.county_code = ''
    ...
    fetchData() {
      httpRequest('/api/sys/school/lookup', this.search_school, false).then(res => {
