          ...
          <label class="label-item" :for="'file' + index">
            ...
            <div class="name_item">
              <div class="left_name">
                ...
                <div class="phone">{{ item.tea_phone }}</div>
              </div>
...
export default {
  ...
  created() {
    this.search.scl_id = sessionStorage.getItem('scl_id')
  },
  mounted() {
    this.handleGetData()
  },
  methods: {
    ...
    handleGetData() {
      httpFormPostMethod('web/photocollection/getTeaList', this.search).then((res) => {
        this.list = res.data || []
      })
    },
    async handlePortrait(data, index, name) {
      ...
      if (file) {
        ...
        httpUploadFile('web/photocollection/uploadByTea', {
          tea_no: data.tea_no,
          img: select_file
        }).then(() => {
