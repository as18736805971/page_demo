<template>
  <div class="calendar_body" ref="calendarBody">
    <div class="calendar_control">
      <img
        src="../assets/pre_calendar.png"
        alt=""
        class="control_img"
        @click="pre_calendar"
      />
      <span class="date">{{ checkedDateStr }}</span>
      <img
        src="../assets/next_calendar.png"
        alt=""
        class="control_img"
        @click="next_calendar"
      />
    ...
    >
      ...
      >
        ...
        >
          <div
            :style="{
              transform: `translate3d(0, ${calendarY}px, 0)`,
              transitionDuration: `${
                isTouching || isWeekSwitch ? 0 : transitionDuration
              }s`,
            }"
            ...
            >
              <p
                :class="{
                  ...
                  calendar_day_not:
                    !isShowWeek && isNotCurrentMonthDay(date, i),
                }"
                class="calendar_day"
                ref="calendarDay"
              >
    ...
    <div class="calendar_bot" @click="open_calendar">
      <img
        src="../assets/calendar_open.png"
        alt=""
        class="is_open"
        v-show="isShowWeek"
      />
      <img
        src="../assets/calendar_close.png"
        alt=""
        class="is_open"
        v-show="!isShowWeek"
      />
...
export default {
  ...
  watch: {
    ...
    checkedDate: {
      handler(val) {
        let year = val.year.toString();
        let month =
          val.month + 1 < 10
            ? "0" + (val.month + 1).toString()
            : (val.month + 1).toString();
        let date = val.day < 10 ? "0" + val.day.toString() : val.day.toString();
        this.$emit("confirm", year + month + date);
      },
      deep: true,
      immediate: true,
    },
    ...
    calendarGroupHeight(val) {
      ...
      this.$emit("weekshow", this.isWeekSwitch);
    },
    currentMonthRange: {
      handler(val, oldVal) {
  ...
  methods: {
    ...
    showWeek() {
      ...
      if (
        ...
        lastDayOfCurrentWeek.month === this.checkedDate.month
      ) {
        ...
        if (firstDayOfCurrentWeek.day === 1) {
          ...
          if (
            this.lastWeek[this.selectedDayIndex].month ===
            this.checkedDate.month
          ) {
      ...
      if (
        ...
        if (
          ...
          if (
            this.nextWeek[this.selectedDayIndex].month ===
            this.checkedDate.month
          ) {
    ...
    open_calendar() {
      ...
      this.$emit("ifshow_week", this.isWeekSwitch);
    },
    pre_calendar() {
      if (this.isWeekSwitch) {
        this.getLastMonth();
