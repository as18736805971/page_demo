import { getToken, getSign, getAppSecret } from '@/utils/auth'
...
export function formatTree(data, id) {
  var itemArr = []
  data.forEach(function(val, index) {
    if (val.pid === id) {
      var newNode = {
        ...
        pid: val.pid,
        child_menu: formatTree(data, val.id)
      }
...
export function formatArea(city_data) {
  var city_list = []
  for (var i in city_data) {
    if (!city_data[i].area_pcode) {
      city_list[city_data[i].area_code] = {
        area_code: city_data[i].area_code,
        area_name: city_data[i].area_name,
        sub_area: []
      }
    } else {
      if (!city_list.hasOwnProperty(city_data[i].area_pcode)) {
        city_list[city_data[i].area_pcode] = {
          area_code: '',
          area_name: '',
          sub_area: []
        }
      }
      city_list[city_data[i].area_pcode].sub_area.push({ area_code: city_data[i].area_code, area_name: city_data[i].area_name })
    }
  }
  return city_list
}
...
export function uploadFile(data, file, method) {
