<template>
  <div class="app-container page-container right-side-dialog">
    ...
    <div class="main-header">
      <div class="left">
        <span
          v-if="functionList.includes('A40016')"
          class="spp-btn spp-btn-primary"
          @click="addAudio()"
        >
          <svg-icon icon-class="add" />新增
        </span>
        <span v-if="functionList.includes('A40018')" class="spp-btn" @click="delBatch()">
    ...
    <div class="main-body">
      <el-table ref="table" :data="data">
        ...
        <el-table-column label="设备安装位置" prop="equ_position" />
        ...
        <el-table-column prop="address" label="操作" width="130">
          <template slot-scope="scope">
            <el-button v-if="functionList.includes('A40017')" type="text" @click="edit(scope.row)">编辑</el-button>
            <el-button v-if="functionList.includes('A40018')" type="text" @click="del(scope.row.equ_id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <pagination
    ...
    >
      <el-scrollbar wrap-class="vertical_scrollbar">
        <el-form v-if="showDialog" ref="form" :model="form" :rules="rules" label-width="120px">
          ...
          <el-form-item label="设备安装位置：" prop="equ_position">
            <el-input v-model="form.equ_position" placeholder="请输入设备安装位置" />
          </el-form-item>
...
import { paginate, save, update, del } from '@/api/device_manage_visitor'
