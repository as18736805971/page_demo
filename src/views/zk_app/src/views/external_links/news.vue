<template>
  <div class="detail_index">
    ...
    <template v-else>
      <div class="no-data">未查询到相关校园新闻</div>
    </template>
  </div>
</template>
...
export default {
  ...
  methods: {
    handleGetDetail() {
      ...
      const url = process.env.VUE_APP_BASE_API + '/api/dd/scl/article/detail'
      axios({
        method: 'post',
        url: url,
        data,
        ...
        if (Number(info.status) === 200) {
          document.getElementsByTagName('title')[0].innerText = info.data.title || '校园新闻'
