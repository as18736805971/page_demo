<template>
  <div class="app-container page-container">
    <div class="main-header">
      <div class="left">
        <span class="spp-btn spp-btn-primary" @click="addUser()">
          <svg-icon v-if="functionList.includes('A11001')" icon-class="add" />新增
        </span>
      </div>
      <div>
        <div class="input-box">
        ...
        <el-table-column prop="address" label="操作" width="130">
          <template slot-scope="scope">
            <div v-if="scope.row.isadmin==='1'">
              <el-button v-if="functionList.includes('A11004')" type="text" @click.native="initpwdUser(scope.row)">初始化密码</el-button>
            </div>
            <div v-else>
              <el-button
                v-if="functionList.includes('A11002')"
                type="text"
              ...
              <el-button type="text">
                <el-dropdown>
                  ...
                  <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item v-if="functionList.includes('A11004')" @click.native="initpwdUser(scope.row)">初始化密码</el-dropdown-item>
                    <el-dropdown-item
                      v-if="functionList.includes('A11003')"
...
export default {
  ...
  methods: {
    ...
    getData() {
      userPaginate(this.search, this.page, this.page_size).then(response => {
    ...
    deleteUser(row) {
      ...
      this.$confirm('您确定要删除该用户吗？', '删除提示', {
        ...
        .then(() => {
          deleteEduuser(user_id).then(res => {
            this.$message({
              message: '删除成功',
              type: 'success',
              duration: 3000
    ...
    initpwdUser(row) {
      const { user_id } = row
      this.$confirm('您确定要初始化密码吗？', '删除提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      })
        .then(() => {
          initpwd(user_id).then(res => {
            this.$message({
              message: '初始化成功',
              type: 'success',
              duration: 3000
    ...
    submit() {
      if (!this.user_id) {
        ...
        this.$refs.form.validate(valid => {
          if (valid) {
            addEduuser(this.form).then(response => {
