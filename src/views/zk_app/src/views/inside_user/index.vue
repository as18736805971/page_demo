<template>
  <div class="app-container page-container">
    <div class="main-header">
      <div class="left">
        <span v-if="functionList.includes('A07001')" class="spp-btn spp-btn-primary" @click="addUser()"><svg-icon icon-class="add" /><span>新增</span></span>
      </div>
      <div>
        <div class="input-box">
          <input v-model="search" placeholder="搜索姓名或手机号" style="width:280px;" @keyup.enter="searchData()">
      ...
      <el-table ref="multipleTable" :data="userData">
        ...
        <el-table-column prop="address" label="操作" width="130">
          <template slot-scope="scope">
            <el-button v-if="scope.row.status_flag == '1' && functionList.includes('A07003')" class="red" type="text" @click="disableUser(scope.row)">停用</el-button>
            <el-button v-if="scope.row.status_flag != '1' && functionList.includes('A07004')" class="green" type="text" @click="enableUser(scope.row)">启用</el-button>
            <el-button v-if="functionList.includes('A07002')" type="text" @click="editUser(scope.row)">编辑</el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px" class="model_form insider_form">
        <el-form-item label="角色" prop="user_role">
          <el-select v-model="form.user_role" placeholder="请选择角色">
            <el-option v-for="(role, i) in roles" :key="i" :label="role" :value="i" />
          </el-select>
        </el-form-item>
...
<script>
import { paginate, disableUser, enableUser, addUser, editUser, getUser } from '@/api/inside_user'
...
export default {
    ...
    return {
      ...
      rules: {
        user_name: [
          ...
          { min: 1, max: 10, message: '姓名长度在 1 到 10 个字之间', trigger: 'blur' }
        ],
        user_phone: [
  ...
  methods: {
    ...
    disableUser(row) {
      const { user_id } = row
      disableUser(user_id).then(response => {
        this.$message({
          message: '操作成功',
          type: 'success',
          duration: 3000
    ...
    enableUser(row) {
      const { user_id } = row
      enableUser(user_id).then(response => {
        this.$message({
          message: '操作成功',
          type: 'success',
          duration: 3000
    ...
    editUser(row) {
      ...
      getUser(user_id).then(response => {
        ...
        this.form = {
          ...
          user_role: this.roles.hasOwnProperty(data.role_id) ? data.role_id : ''
        }
        this.reset()
        this.showDialog = true
      })
